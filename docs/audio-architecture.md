...
描画を待たせず後ろで進める。完了していないパッドは初回再生時に遅延デコードされる。
デコード済みバッファはシーンをまたいで保持し、再訪シーンの再デコードを避ける。
未キャッシュのパッドにはダウンロードアイコンを表示し、Shift+クリックで
その音源を LRU の最新位置へ明示的にキャッシュできる。
`decodedAudioBuffers` は合計 `AUDIO_MEMORY_BUDGET_BYTES` (2GB) の予算内で管理し、
超過時は再生中・一時停止中 (pinned) を除いて LRU 順に解放する。解放は
現在のシーン以外のバッファを優先し (1パス目)、それでも超過する場合のみ
現在シーンのバッファも解放する (2パス目)。シーン再訪時の再ウォームアップが
自分自身の古いバッファと入れ替わる消耗を防ぐため。事前デコードは予算が
飽和した時点で打ち切り、残りは初回再生時の遅延デコードに任せる。解放時に
